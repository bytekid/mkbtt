...
(* Format.printf "File exists: %i\n%!" (if Sys.file_exists filename then 1 else 0);
 Sys.command ("cat "^filename);*)
 let itimeout = truncate (ceil timeout) in
 let cmd = cmd^filename^" "^(string_of_int itimeout) in
(* Format.printf "%s\n" cmd;*)
...
let certify trs =
 ...
 let filename = write_to_tmp s in
 let cmd = "./callcertify "^filename in
 (*Format.printf "%s\n" cmd;*)
 let t = Process.Local 30.0 in
 match Process.run_timed t syscall cmd with
 | Some (r,e), time ->
  ((*remove_tmp_file filename;*)
  if String.length r > 2 then return r
  else
   ((*Format.printf "UNKNOWN\n%!";*) return ""))
 | None,_ -> ((*Format.printf "TMO\n%!";*) remove_tmp_file filename; 
  return "")
...
let check_parallel' trs0 trs1 =
 ...
 Statistics.inc_n_termination_calls >>
 Statistics.inc_n_termination_calls >>
 W.M.Trsx.to_wst_stringm trs0 >>= fun s0 ->
 W.M.Trsx.to_wst_stringm trs1 >>= fun s1 ->
 let check s = run_external2 cmd s in
 match Process.run_parallel [check s0; check s1] timeout with
