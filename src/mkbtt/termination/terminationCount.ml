...
(* check if trs_p (the constraint system for p) terminates together
   ...
   Was already checked before if new rule is real rewrite rule *)
let add_rule_ok rule trs  =
 if CountingFuns.decreasing (Trs.add rule trs) then
  return true
 else
...
let trss_terminate trs0 trs1 =
 W.get_options >>= fun o ->
 if Completion.check_externally o then
  failwith "External checking does not support parallelism"
 else
  InterfaceTTT.check_both_parallel trs0 trs1
;;

(* check if trs_p (the constraint system for p) terminates together
   with a) the rule s->t, b) the rule t-> s. *)
let add_rules_ok st ts trs  = 
 let trs0, trs1 = Trs.add st trs, Trs.add ts trs in
 let dec0, dec1 = Pair.map CountingFuns.decreasing (trs0,trs1) in
 if dec0 && dec1 then return (true,true)
 else if dec0 then
  add_rule_ok ts trs >>= fun t1 -> return (dec0, t1)
 else if dec1 then
  add_rule_ok st trs >>= fun t0 -> return (t0, dec1)
 else
  trss_terminate (Trs.add st trs) (Trs.add ts trs)
;;

let preserved_parallel (st, ts) trs =
 W.get_options >>= fun o ->
 if Completion.check_externally o then
  preserved (st, ts) trs
 else (
  add_rules_ok st ts trs
 )
;;
