...
module Imperative(X : Ordered) = struct
  ...
  type t = { mutable size : int; mutable data : X.t array; mutable count : int}
  ...
  let load h (s,c,d) = 
    ...
    h.count <- c;
    h.data <- Array.copy d
(*     { size = s; data = d; count = 0} *)

  let dump h = 
    h.size,h.count,h.data

  let clone h f = 
    f (dump h)

  let clear h = h.size <- -1; h.count <- 0
  ...
  let pop h = let m = peek h in remove h; m
