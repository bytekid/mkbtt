...
let add_t_srewrite t =
 update (fun c -> {c with t_mkb_srewrite = c.t_mkb_srewrite +. t})
;;
...
let add_cp_for_processes pset = 
 if List.is_empty pset then return () else
 inc_n_redundant_cps >>
...
let index =
 ...
 get_statistics >>= fun c -> return (
  ...
  Format.printf "    One: %f\n%!" c.t_one;
  Format.printf "    Two: %f\n%!" c.t_two;(*
  Format.printf "    rew1: %f\n" !ACRewrite.t_rewrite1;
  Format.printf "    rew2: %f\n" !ACRewrite.t_rewrite2; 
  Format.printf "    match: %f\n" !ACRewrite.t_match;
  Format.printf "    solve: %f\n" !ACLogic.t_solve;
  Format.printf "    mcount: %i (%i repetitions)\n%!" !ACRewrite.mcount !ACRewrite.mrepcount;*)
)
;;
...
let cpc p =
 ...
 get_statistics >>= fun c -> 
 let cpc = C.cpc o in
 let s = match cpc with
  | C.Prime -> "primality"
  | C.SReducibility -> "S-reducibility"
  | C.Mixed -> "mixed"
  | _ -> "connectedness"
 in
 if cpc <> C.NoCPC then
  (
  Format.printf "\nCritical pair criterion (%s) \n" s;
  Format.printf " redundant CPs in total:  %i\n%!" c.n_redundant_cps; 
  get_redundant_cps_for p >>= fun n ->
  Format.printf " for successful process:  %i\n%!" n;
  return (Format.printf " time to check:      %.2f\n" c.t_cpc)
  ) else return ()
;;
...
let mkb_general =
 ...
 Format.printf " time for S-rewrite:      %.2f\n" c.t_mkb_srewrite;
...
let mkb_theory =
 get_options >>= fun o ->
 Format.printf "Theory\n";
 Format.printf " convergent system S:     %s \n" (C.s_theory_file o);
 return ()
;;
...
let print_system r_omega =
 M.Trs.to_stringm r_omega >>= fun s ->
 Format.printf "S-CONVERGENT TRS: \n%s\n\n%!" s;
 return ()
;;

let goal_to_string = function
 | None -> "" 
 | Some e -> Equation.to_string e
;;
